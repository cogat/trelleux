...
import pytz
from lib.lis import longest_increasing_subsequence
...
def update_lists(client, board_id, days_in_future, timezone):
    ...
    for missing_date in missing_dates:
        ...
        if missing_date == local_today:
            ...
            existing_dates[local_today] = client.post(
                "lists", f"name={datestr}&idBoard={board_id}&pos=bottom"
            )
        else:
            p = Process(
                target=client.post,
                args=("lists", f"name={datestr}&idBoard={board_id}&pos=bottom"),
            )
            processes.append(p)
            p.start()
    ...
    for d, l in past_lists.items():
        ...
        # archive the list
        p = Process(target=client.put, args=(f'lists/{l["id"]}', "closed=true"))
        processes.append(p)
        p.start()
    ...
    if alexa_list:
        ...
        p = Process(
            target=client.put, args=(f'lists/{alexa_list["id"]}/pos', "value=bottom")
        )
        processes.append(p)
        p.start()
...
def sort_lists(client, board_id):
    ...
    print("sorting")
    trello_lists = client.get(f"boards/{board_id}/lists")
    list_names = []
    list_data = {}
    ...
    for name_to_move in names_to_move:
        ...
        p = Process(
            target=client.put,
            args=(f'lists/{list_data[name_to_move]["id"]}', f"pos={new_pos}"),
        )
        processes.append(p)
        p.start()
...
def archive_empty_lists(client, board_id):
    ...
    lists = client.get(f"boards/{board_id}/lists", "cards=open")
    processes = []
    for trello_list in lists:
        if not trello_list["cards"]:
            # archive the list
            p = Process(
                target=client.put, args=(f'lists/{trello_list["id"]}', "closed=true")
            )
            processes.append(p)
            p.start()
    for process in processes:
...
def lambda_func(event, context):
    ...
    if "archive_empty_lists" in event and event["archive_empty_lists"]:
        archive_empty_lists(client, event["board_id"])
    update_lists(
        client=client,
    ...
    sort_lists(client, event["board_id"])
